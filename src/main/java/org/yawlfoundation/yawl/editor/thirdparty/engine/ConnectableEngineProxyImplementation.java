...
public class ConnectableEngineProxyImplementation extends AvailableEngineProxyImplementation {
    ...
    public LinkedList getEngineParametersForRegisteredService(String registeredYAWLServiceURI) {
      ...
      if (connected()) {
        ...
        YParameter[] engineParametersForService = null;

        try {
        	if (registeredService.getScheme().equals("http"))
        	{
            engineParametersForService = new InterfaceB_EngineBasedClient().getRequiredParamsForService(
                registeredService
            );
        	}
        	if (registeredService.getScheme().equals("tcp"))
        	{
                engineParametersForService = new InterfaceB_EngineBased_ZMQ_Client().getRequiredParamsForService(
                        registeredService
                    );
        	}
        } catch (Exception ioe) {
